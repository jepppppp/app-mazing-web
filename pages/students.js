...
const Students = () => {
  ...
  const load = async () => {
    const { success, data } = await getAllUser(2);
  ...
  return (
    <>
      <LoggedInNavbar activePage="students" />
    ...
    <Modal>
      ...
      >
        ...
        <form onSubmit={submitHandler} className="flex flex-col gap-2">
          <div>
            <label className="inline-block py-2">LRN:</label>
            <input
              required
              type="text"
              ref={idNumberRef}
          ...
          <div>
            <label className="inline-block py-2">Grade Level:</label>
            <select
              ref={gradeRef}
              defaultValue={data?.grade}
              className="capitalize -outline-offset-1 p-2 px-4 rounded-md  bg-gray-100 w-full "
            >
              {grade_level.map((item, key) => (
                <option key={key} value={item} className="capitalize">
                  {item}
                </option>
              ))}
            </select>
